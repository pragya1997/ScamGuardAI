...
class ScamDetector:
    """Orchestrates the scam detection process using LLMExecutor and OutputParser."""

    def __init__(self, strategy : str = "react")->None:
        self.executor = LLMExecutor()
        self.parser = OutputParser()
        self.strategy = strategy
        logger.info(f"ScamDetector initialized with strategy: {self.strategy}")
    ...
    def detect(self, message: str)-> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Detection completed with Result: {parsed_result}")
    ...
    def detect_batch(self, messages: List[str]) -> List[Dict[str, Any]]:
        """Runs scam detection on a batch of messages."""
        ...
        for i , message in enumerate(messages):
            try:
                ...
                logger.warning(f"Failed to process message {i+1}/{total_messages}: {str(e)}")
                #Append a fallback result for this message
                error_result = {
                    "label": "Uncertain",
        ...
        logger.info(f"Batch detection completed. Successful detections: {successful}/{total_messages}")
        return results